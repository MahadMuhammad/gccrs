...
fn no_hrtb<'b, T>(mut t: T) // { dg-warning "" "" { target *-*-* } }
where
    T: Bar<&'b isize>,
{
...
fn bar_hrtb<T>(mut t: T) // { dg-warning "" "" { target *-*-* } }
where
    T: for<'b> Bar<&'b isize>,
{
...
fn foo_hrtb_bar_not<'b, T>(mut t: T) // { dg-warning "" "" { target *-*-* } }
where
...
fn foo_hrtb_bar_hrtb<T>(mut t: T) // { dg-warning "" "" { target *-*-* } }
where
