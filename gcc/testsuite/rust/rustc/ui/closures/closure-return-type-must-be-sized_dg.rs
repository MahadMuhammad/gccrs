...
fn main() {
    a::foo::<fn() -> dyn A>();         // { dg-error ".E0277." "" { target *-*-* } }
    a::bar::<fn() -> dyn A, _>();      // { dg-error ".E0277." "" { target *-*-* } }
    a::baz::<fn() -> dyn A>();         // { dg-error ".E0277." "" { target *-*-* } }
    a::foo::<fn() -> Box<dyn A>>();    //  ok
    a::bar::<fn() -> Box<dyn A>, _>(); //  ok
    a::baz::<fn() -> Box<dyn A>>();    //  ok

    b::foo::<fn() -> dyn A>();         // { dg-error ".E0277." "" { target *-*-* } }
    b::bar::<fn() -> dyn A, _>();      // { dg-error ".E0277." "" { target *-*-* } }
    b::baz::<fn() -> dyn A>();         // { dg-error ".E0277." "" { target *-*-* } }
    b::foo::<fn() -> Box<dyn A>>();    //  ok
    b::bar::<fn() -> Box<dyn A>, _>(); //  ok
    b::baz::<fn() -> Box<dyn A>>();    //  ok

    c::foo::<fn() -> dyn A>();         // { dg-error ".E0277." "" { target *-*-* } }
    c::bar::<fn() -> dyn A, _>();      // { dg-error ".E0277." "" { target *-*-* } }
    c::baz::<fn() -> dyn A>();         // { dg-error ".E0277." "" { target *-*-* } }
