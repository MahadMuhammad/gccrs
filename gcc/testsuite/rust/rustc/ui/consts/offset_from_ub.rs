...
pub const DIFFERENT_ALLOC: usize = {
    ...
    let offset = unsafe { ptr_offset_from(field_ptr, base_ptr) }; // { dg-error ".E0080." "" { target *-*-* } }
// { dg-error "" "" { target *-*-* } .-2 }
    offset as usize
};
...
pub const NOT_MULTIPLE_OF_SIZE: isize = {
    ...
    unsafe { ptr_offset_from(field_ptr, base_ptr as *const u16) } // { dg-error ".E0080." "" { target *-*-* } }
// { dg-error "" "" { target *-*-* } .-2 }
};
...
pub const DIFFERENT_INT: isize = { // offset_from with two different integers: like DIFFERENT_ALLOC
    ...
    unsafe { ptr_offset_from(ptr2, ptr1) } // { dg-error ".E0080." "" { target *-*-* } }
// { dg-error "" "" { target *-*-* } .-2 }
};
...
const OUT_OF_BOUNDS_1: isize = {
    ...
    unsafe { ptr_offset_from(end_ptr, start_ptr) } // { dg-error ".E0080." "" { target *-*-* } }
// { dg-error "" "" { target *-*-* } .-2 }
};
...
const OUT_OF_BOUNDS_2: isize = {
    ...
    unsafe { ptr_offset_from(start_ptr, end_ptr) } // { dg-error ".E0080." "" { target *-*-* } }
// { dg-error "" "" { target *-*-* } .-2 }
};
...
pub const DIFFERENT_ALLOC_UNSIGNED: usize = {
    ...
    let uninit2 = std::mem::MaybeUninit::<Struct>::uninit();
    let field_ptr: *const Struct = &uninit2 as *const _ as *const Struct;
    unsafe { ptr_offset_from_unsigned(field_ptr, base_ptr) } // { dg-error ".E0080." "" { target *-*-* } }
// { dg-error "" "" { target *-*-* } .-2 }
};
...
pub const TOO_FAR_APART2: isize = {
    let ptr1 = &0u8 as *const u8;
    let ptr2 = ptr1.wrapping_add(isize::MAX as usize + 42);
    unsafe { ptr_offset_from(ptr1, ptr2) } // { dg-error ".E0080." "" { target *-*-* } }
// { dg-error "" "" { target *-*-* } .-2 }
};
...
const WRONG_ORDER_UNSIGNED: usize = {
    let a = ['a', 'b', 'c'];
    let p = a.as_ptr();
    unsafe { ptr_offset_from_unsigned(p, p.add(2) ) } // { dg-error ".E0080." "" { target *-*-* } }
// { dg-error "" "" { target *-*-* } .-2 }
};
pub const TOO_FAR_APART_UNSIGNED: usize = {
    ...
    unsafe { ptr_offset_from_unsigned(ptr2, ptr1) } // { dg-error ".E0080." "" { target *-*-* } }
// { dg-error "" "" { target *-*-* } .-2 }
};
