...
const MUT: Option<&mut i32> = helper(); // { dg-error ".E0080." "" { target *-*-* } }
...
const INT2PTR: Option<&mut i32> = helper_int2ptr(); // { dg-error ".E0080." "" { target *-*-* } }
// { dg-error ".E0080." "" { target *-*-* } .-1 }
static INT2PTR_STATIC: Option<&mut i32> = helper_int2ptr(); // { dg-error ".E0080." "" { target *-*-* } }
// { dg-error ".E0080." "" { target *-*-* } .-1 }
...
const DANGLING: Option<&mut i32> = helper_dangling(); // { dg-error ".E0080." "" { target *-*-* } }
static DANGLING_STATIC: Option<&mut i32> = helper_dangling(); // { dg-error ".E0080." "" { target *-*-* } }
