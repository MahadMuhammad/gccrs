...
expand_expr_fail!("string"; hello); // { dg-error "" "" { target *-*-* } }

// Invalid expressions produce errors in addition to returning `Err(())`.
expand_expr_fail!($); // { dg-error "" "" { target *-*-* } }
expand_expr_fail!(echo_tts!($)); // { dg-error "" "" { target *-*-* } }
expand_expr_fail!(echo_pm!($)); // { dg-error "" "" { target *-*-* } }

// We get errors reported and recover during macro expansion if the macro
// doesn't produce a valid expression.
expand_expr_is!("string", echo_tts!("string"; hello)); // { dg-error "" "" { target *-*-* } }
expand_expr_is!("string", echo_pm!("string"; hello)); // { dg-error "" "" { target *-*-* } }
...
const _: u32 = recursive_expand!(); // { dg-error "" "" { target *-*-* } }
