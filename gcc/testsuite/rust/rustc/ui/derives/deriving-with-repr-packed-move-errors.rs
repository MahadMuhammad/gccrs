...
// { dg-error ".E0507." "" { target *-*-* } .-5 }
// { dg-error ".E0507." "" { target *-*-* } .-6 }
// { dg-error ".E0507." "" { target *-*-* } .-7 }
// { dg-error ".E0507." "" { target *-*-* } .-8 }
// { dg-error ".E0507." "" { target *-*-* } .-9 }


// Unrelated impl: additinal diagnostic should NOT be emitted
impl StructA {
    fn fmt(&self) -> String {
        self.0 // { dg-error ".E0507." "" { target *-*-* } }
    }
}
...
impl Debug for StructB {
    fn fmt(&self, f: &mut Formatter) -> Result {
        let x = &{ self.0 }; // { dg-error ".E0507." "" { target *-*-* } }
        write!(f, "{}", x)
    }
}
