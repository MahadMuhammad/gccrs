...
#[cfg(neg)]
fn env0<'any>() {
    let _: TypeOutlives<'static, &'any ()>; // { dg-error "" "" { target *-*-* } }
}

#[cfg(neg)]
fn env1<'any>() {
    let _: RegionOutlives<'static, 'any>; // { dg-error "" "" { target *-*-* } }
}

#[cfg(neg)]
fn env2<'any>() {
    let _: Outer0<'static, &'any ()>; // { dg-error "" "" { target *-*-* } }
}

#[cfg(neg)]
fn env3<'any>() {
    let _: Outer1<'static, &'any ()>; // { dg-error "" "" { target *-*-* } }
}
