...
fn main() {
    ...
    let ref_ascribe = &raw const type_ascribe!(2, i32);             // { dg-error ".E0745." "" { target *-*-* } }
    let mut_ref_ascribe = &raw mut type_ascribe!(3, i32);           // { dg-error ".E0745." "" { target *-*-* } }

    let ascribe_field_ref = &raw const type_ascribe!(PAIR.0, i32);  // { dg-error ".E0745." "" { target *-*-* } }
    let ascribe_index_ref = &raw mut type_ascribe!(ARRAY[0], i32);  // { dg-error ".E0745." "" { target *-*-* } }
}
