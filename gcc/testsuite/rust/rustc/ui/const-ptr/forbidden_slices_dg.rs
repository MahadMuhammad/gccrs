...
pub static S4: &[u8] = unsafe { from_raw_parts((&D1) as *const _ as _, 1) }; // { dg-error ".E0080." "" { target *-*-* } }
// Reinterpret pointers as integers (UB in CTFE.)
pub static S5: &[u8] = unsafe { from_raw_parts((&D3) as *const _ as _, size_of::<&u32>()) }; // { dg-error ".E0080." "" { target *-*-* } }
// Layout mismatch
pub static S6: &[bool] = unsafe { from_raw_parts((&D0) as *const _ as _, 4) }; // { dg-error ".E0080." "" { target *-*-* } }
...
pub static R8: &[u64] = unsafe {
    let ptr = (&D4 as *const [u32; 2] as *const u32).byte_add(1).cast::<u64>();
    from_ptr_range(ptr..ptr.add(1)) // { dg-error "" "" { target *-*-* } }
};
