...
fn test_0000_local_fully_init_and_use_struct() {
    let s: S<B>;
    s.x = 10; s.y = Box::new(20); // { dg-error ".E0381." "" { target *-*-* } }
    use_fully!(struct s);
}

fn test_0001_local_fully_init_and_use_tuple() {
    let t: T;
    t.0 = 10; t.1 = Box::new(20); // { dg-error ".E0381." "" { target *-*-* } }
    use_fully!(tuple t);
}
...
fn test_0100_local_partial_init_and_use_struct() {
    let s: S<B>;
    s.x = 10; // { dg-error ".E0381." "" { target *-*-* } }
    use_part!(struct s);
}

fn test_0101_local_partial_init_and_use_tuple() {
    let t: T;
    t.0 = 10; // { dg-error ".E0381." "" { target *-*-* } }
    use_part!(tuple t);
}
...
fn test_1000_field_fully_init_and_use_struct() {
    let q: Q<S<B>>;
    q.r.f.x = 10; q.r.f.y = Box::new(20); // { dg-error ".E0381." "" { target *-*-* } }
    use_fully!(struct q.r.f);
}

fn test_1001_field_fully_init_and_use_tuple() {
    let q: Q<T>;
    q.r.f.0 = 10; q.r.f.1 = Box::new(20); // { dg-error ".E0381." "" { target *-*-* } }
