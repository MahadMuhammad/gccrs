...
fn all_patterns_are_tested() {
    ...
    let x = String::new();
    if let _ = true { // { dg-warning "" "" { target *-*-* } }
    } else {
        drop(x)
    }
...
fn guard_may_be_skipped(y: i32) {
    ...
    match y {
        ...
        _ => drop(x), // { dg-error ".E0381." "" { target *-*-* } }
    };
    ...
    match y {
        _ if { x = 2; true } => 1,
        // Borrowck must not know the guard is always run.
        _ if { x; false } => 2, // { dg-error ".E0381." "" { target *-*-* } }
        _ => 3,
    };
    ...
    match y {
        ...
        _ if let Some(()) = { x; None } => 2, // { dg-error ".E0381." "" { target *-*-* } }
        _ => 3,
    };
}
...
fn guard_may_be_taken(y: bool) {
    ...
    match y {
        ...
        true => drop(x), // { dg-error ".E0382." "" { target *-*-* } }
        false => {},
    };
