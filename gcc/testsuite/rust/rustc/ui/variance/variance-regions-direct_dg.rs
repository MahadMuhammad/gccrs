...
#[rustc_variance]
struct Test2<'a, 'b, 'c> { // { dg-error "" "" { target *-*-* } }
    x: &'a isize,
    y: &'b [isize],
    c: &'c str
}
...
#[rustc_variance]
struct Test3<'a, 'b, 'c> { // { dg-error "" "" { target *-*-* } }
...
#[rustc_variance]
struct Test4<'a, 'b:'a> { // { dg-error "" "" { target *-*-* } }
    x: &'a mut &'b isize,
}
...
#[rustc_variance]
struct Test5<'a, 'b:'a> { // { dg-error "" "" { target *-*-* } }
...
#[rustc_variance]
struct Test6<'a, 'b:'a> { // { dg-error "" "" { target *-*-* } }
...
#[rustc_variance]
struct Test7<'a> { // { dg-error ".E0392." "" { target *-*-* } }
...
#[rustc_variance]
enum Test8<'a, 'b, 'c:'b> { // { dg-error "" "" { target *-*-* } }
