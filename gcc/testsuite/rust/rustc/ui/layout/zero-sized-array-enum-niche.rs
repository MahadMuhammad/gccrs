...
#[rustc_layout(debug)]
type AlignedResult = Result<[u32; 0], bool>; // { dg-error "" "" { target *-*-* } }
...
#[rustc_layout(debug)]
enum MultipleAlignments { // { dg-error "" "" { target *-*-* } }
    Align2([u16; 0]),
    Align4([u32; 0]),
    Niche(bool),
}
...
#[rustc_layout(debug)]
type NicheLosesToTagged = Result<[u32; 0], Packed<std::num::NonZero<u16>>>; // { dg-error "" "" { target *-*-* } }
...
#[rustc_layout(debug)]
type NicheWinsOverTagged = Result<[u32; 0], Packed<U16IsZero>>; // { dg-error "" "" { target *-*-* } }
