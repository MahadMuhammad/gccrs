...
macro_rules! foo {
    ...
    (tt $x:tt) => { bar!(tt $x); };
    (expr $x:expr) => { bar!(expr $x); }; // { dg-error "" "" { target *-*-* } }
    (literal $x:literal) => { bar!(literal $x); }; // { dg-error "" "" { target *-*-* } }
    (path $x:path) => { bar!(path $x); }; // { dg-error "" "" { target *-*-* } }
    (stmt $x:stmt) => { bar!(stmt $x); }; // { dg-error "" "" { target *-*-* } }
}
