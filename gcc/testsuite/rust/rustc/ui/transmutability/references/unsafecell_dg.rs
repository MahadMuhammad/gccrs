...
fn ref_to_ref() {
    ...
    assert::is_maybe_transmutable::<&'static u8, &'static UnsafeCell<u8>>(); // { dg-error ".E0277." "" { target *-*-* } }

    assert::is_maybe_transmutable::<&'static UnsafeCell<u8>, &'static UnsafeCell<u8>>(); // { dg-error ".E0277." "" { target *-*-* } }
}
