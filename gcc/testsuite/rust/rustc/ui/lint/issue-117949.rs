...
fn main() {
    format_args!("{}", 1 << 32); // { dg-error "" "" { target *-*-* } }
    format_args!("{}", 1 >> 32); // { dg-error "" "" { target *-*-* } }
    format_args!("{}", 1 + i32::MAX); // { dg-error "" "" { target *-*-* } }
    format_args!("{}", -5 - i32::MAX); // { dg-error "" "" { target *-*-* } }
    format_args!("{}", 5 * i32::MAX); // { dg-error "" "" { target *-*-* } }
    format_args!("{}", 1 / 0); // { dg-error "" "" { target *-*-* } }
    format_args!("{}", 1 % 0); // { dg-error "" "" { target *-*-* } }
    format_args!("{}", [1, 2, 3][4]); // { dg-error "" "" { target *-*-* } }
}
