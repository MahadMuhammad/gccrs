...
fn main() {
    drop(&SomeStruct); // { dg-warning "" "" { target *-*-* } }

    let mut owned1 = SomeStruct;
    drop(&owned1); // { dg-warning "" "" { target *-*-* } }
    drop(&&owned1); // { dg-warning "" "" { target *-*-* } }
    drop(&mut owned1); // { dg-warning "" "" { target *-*-* } }
    drop(owned1);

    let reference1 = &SomeStruct;
    drop(reference1); // { dg-warning "" "" { target *-*-* } }

    let reference2 = &mut SomeStruct;
    drop(reference2); // { dg-warning "" "" { target *-*-* } }

    let ref reference3 = SomeStruct;
    drop(reference3); // { dg-warning "" "" { target *-*-* } }
}

#[allow(dead_code)]
fn test_generic_fn_drop<T>(val: T) {
    drop(&val); // { dg-warning "" "" { target *-*-* } }
    drop(val);
}
...
fn test_similarly_named_function() {
    ...
    std::mem::drop(&SomeStruct); // { dg-warning "" "" { target *-*-* } }
}
...
fn issue10122(x: u8) {
    ...
    match x {
        ...
        2 => {
            ...
            drop(println_and(&13)); // { dg-warning "" "" { target *-*-* } }
        },
        // Lint, idiomatic use is only in body of `Arm`
        3 if drop(println_and(&14)) == () => (), // { dg-warning "" "" { target *-*-* } }
         // Lint, not a fn/method call
        4 => drop(&2), // { dg-warning "" "" { target *-*-* } }
        _ => (),
    }
}
