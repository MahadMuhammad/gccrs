...
fn main() {
    ...
    // Almost exhaustive
    m!(0u8, 0..255); // { dg-error ".E0004." "" { target *-*-* } }
    m!(0u8, 0..=254); // { dg-error ".E0004." "" { target *-*-* } }
    m!(0u8, 1..=255); // { dg-error ".E0004." "" { target *-*-* } }
    m!(0u8, 0..42 | 43..=255); // { dg-error ".E0004." "" { target *-*-* } }
    m!(0i8, -128..127); // { dg-error ".E0004." "" { target *-*-* } }
    m!(0i8, -128..=126); // { dg-error ".E0004." "" { target *-*-* } }
    m!(0i8, -127..=127); // { dg-error ".E0004." "" { target *-*-* } }
    match 0i8 { // { dg-error ".E0004." "" { target *-*-* } }
        i8::MIN ..= -1 => {}
        1 ..= i8::MAX => {}
    }
    const ALMOST_MAX: u128 = u128::MAX - 1;
    m!(0u128, 0..=ALMOST_MAX); // { dg-error ".E0004." "" { target *-*-* } }
    m!(0u128, 0..=4); // { dg-error ".E0004." "" { target *-*-* } }
    m!(0u128, 1..=u128::MAX); // { dg-error ".E0004." "" { target *-*-* } }
